...
class ComicSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($comics as $comic) {
            ...
            $price = trim($comic['price']);
            $price = floatval(str_replace('$', '', $price));

            $new_comic->title = trim($comic['title']);
            $new_comic->description = trim($comic['description']);
            $new_comic->thumb = trim($comic['thumb']);
            $new_comic->price = $price; // Stringa
            $new_comic->series = trim($comic['series']);
            $new_comic->sale_date= trim($comic['sale_date']);
            $new_comic->type = trim($comic['type']);
